...
# URL: https://drive.google.com/drive/folders/1IV_O1cPFJppKh7iLJdmyMhNLv-j_6XNm
GOOGLE_DRIVE_FOLDER_ID = "1IV_O1cPFJppKh7iLJdmyMhNLv-j_6XNm"

MODEL_DIR = "./model_axis_indobart"  # Lokasi folder model di lokal
MAX_SRC_LENGTH = 64
MAX_TGT_LENGTH = 64
...
def download_model_from_drive():
    ...
    if os.path.exists(MODEL_DIR) and os.path.exists(os.path.join(MODEL_DIR, "config.json")):
        print("Folder model sudah ada. Melanjutkan...")
        return True
    ...
    print("Folder model tidak ditemukan. Mendownload dari Google Drive...")
    ...
    try:
        ...
        with st.spinner("Mendownload model dari Google Drive... (bisa memakan waktu 5-10 menit)"):
...
def load_model_and_tokenizer():
    ...
    if not download_model_from_drive():
        st.error("Gagal memuat model. Periksa koneksi internet dan ID folder.")
        st.stop()
    ...
    try:
        ...
        print(f"Error load tokenizer: {e}")
        print("Mencoba load base tokenizer...")
        tokenizer = IndoNLGTokenizer.from_pretrained(
        ...
        if tokens_to_add:
            ...
            print(f"Ditambahkan {len(tokens_to_add)} token baru")
    ...
    # Resize embedding jika perlu
    if len(tokenizer) != model.config.vocab_size:
        model.resize_token_embeddings(len(tokenizer))
        print(f"Resize embedding: {model.config.vocab_size} -> {len(tokenizer)}")
